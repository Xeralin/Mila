...
from app.constants import BIN_DIR, DD_BIN, DD_URL, DD_ZIP, HM_KEY
from app.style import C, line, step_warn
...
def github_asset(api_url: str, suffix: str) -> tuple[str, str]:
    req = urllib.request.Request(api_url, headers={"User-Agent": "Downloader"})
...
def ensure_depotdownloader(
    reporter: Reporter | None = None, force: bool = False
) -> Path | None:
    ...
    with (reporter or LazySpinner()) as sp:
        ...
        try:
            ...
            with (
                zipfile.ZipFile(DD_ZIP) as z,
                z.open("DepotDownloader") as src,
                open(part, "wb") as f,
            ):
...
def ensure_runtime(
    is_hm: bool, reporter: Reporter | None = None
) -> Path | None:
...
def depot_commands(
    ...
    *,
    ...
    return [
        {
            "args": [
                "-depot", str(depot_id), "-manifest", manifest_id, *common,
            ],
            "name": name,
            "optional": optional,
        }
...
def run_depots(
    ...
    *,
    ...
    for cmd in depot_commands(
        download, steam_account, target, max_downloads, is_hm=is_hm
    ):
