...
def ensure_7zz(
    update: Callable[[str], None], force: bool = False
) -> Path | None:
...
def _sevenz_error(rc: subprocess.CompletedProcess) -> str:
    stderr_line = next(
        (
            line.strip()
            for line in rc.stderr.decode(errors="replace").splitlines()
            if line.strip()
        ),
        "",
    )
...
def ensure_hm_mod(
    hm_version: str, update: Callable[[str], None]
) -> Path | None:
    if hm_version == "latest":
    ...
    try:
        ...
        if _default_args(tmp_dir) is None:
            _set_error(
                f"DefaultArgs.dll missing in Heated Metal {tag} archive"
            )
            return None
        if mod_dir.exists():
...
def screen_update_hm(cfg: dict, downloads: list[dict]) -> None:
    hm_downloads = [
        (target, resolved[0])
        for target in installed_downloads()
        if (resolved := resolve_install(target.name, downloads))
        and resolved[1]
    ]
    if not hm_downloads:
        render_header("Update Heated Metal")
        print()
    ...
    while True:
        labels = []
        for target, _ in hm_downloads:
            name = display_name(target.name, downloads)
            ver = current_hm_version(target)
            labels.append(f"{name:<20}{ver:>8}")
        labels.append("Back")
