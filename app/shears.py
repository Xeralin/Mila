...
def _shears_action(download_dir: Path, kind: str, level: int = 0) -> None:
    ...
    if kind == "videos":
        ...
        step_warn(
            f"This permanently deletes all textures above "
            f"{TEXTURE_QUALITIES[level]}"
        )
    print()
...
def _shears_download(
    download_dir: Path, infos: dict[Path, dict], downloads: list[dict]
) -> None:
    ...
    while True:
        ...
        render_header(f"{mag(name)} — {fmt_bytes(total)} total")
        ...
        if videos > 0:
            actions.append(
                ("Cut videos", lambda: _shears_action(download_dir, "videos"))
            )
        if events > 0:
            actions.append(
                ("Cut events", lambda: _shears_action(download_dir, "events"))
            )
...
def screen_shears(downloads: list[dict]) -> None:
    present = [
        d for d in installed_downloads()
        if not d.name.endswith(HM_FOLDER_SUFFIX)
    ]
    if not present:
        render_header("Shears")
        print()
        step_warn("No Downloader downloads found")
        go_back()
        return
