...
from mila.manifest import local_downloads, load_downloads
...
from mila.heatedmetal import screen_update_heatedmetal
...
def _screen_info(cfg: dict) -> None:
    screen_header("Info")
    ...
    line()
    line(f"Downloads:         {len(local_downloads())}")
    ...
    options = ([f"Update to {pending}"] if pending else []) + ["Back"]
    pick = select("", options, clear_first=False)
    if pick is not None and options[pick] != "Back":
        clear()
        _install_update()
