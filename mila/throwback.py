...
def ensure_throwback(reporter: Reporter | None = None, force: bool = False) -> bool:
...
def write_throwback_toml(target_dir: Path, username: str) -> None:
    dest = target_dir / THROWBACK_TOML
    src = dest if dest.exists() else THROWBACK_DIR / THROWBACK_TOML
    text = src.read_text()
    text = re.sub(
        r"username\s*=\s*'[^']*'",
        f"username = '{username}'",
...
def write_launch_bat(target_dir: Path, liberator_name: str | None = None) -> None:
